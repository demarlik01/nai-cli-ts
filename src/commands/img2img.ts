...
import { buildImg2ImgPayload } from "../novelai/payload/generate.js";
import { parseNovelAiResponse } from "../novelai/response.js";
import type { GlobalCliOptions, Img2ImgCommandOptions } from "../types/commands.js";
...
export function registerImg2ImgCommand(program: Command): void {
  program
    .command("img2img")
    .description("Generate image(s) from an input image and prompt")
    .requiredOption("--image <path>", "Input image path")
    .requiredOption("--prompt <text>", "Prompt text")
    .requiredOption("--strength <number>", "Transformation strength", (value: string) =>
      parseFloatOption(value, "strength"),
    )
    .requiredOption("--noise <number>", "Noise amount", (value: string) =>
      parseFloatOption(value, "noise"),
    )
    ...
    .action(async (options: Img2ImgCommandOptions, command: Command) => {
      ...
      const noise = validateUnitInterval(options.noise, "noise");
